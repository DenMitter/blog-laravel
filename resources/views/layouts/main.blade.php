                ...
                <div class="collapse navbar-collapse" id="edicaMainNav">
                    <ul class="navbar-nav mx-auto mt-2 mt-lg-0">
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="{{ route('main.index') }}">Блог</a>
                        </li>
                    </ul>
                    @auth
                        <ul class="navbar-nav mt-2 mt-lg-0">
                            <li class="nav-item">
                                <form action="{{ route('logout') }}" method="POST">
                                    @csrf
                                    <input class="btn btn-warning font-weight-bold px-5 py-2" type="submit" value="Вийти">
                                </form>
                            </li>
                            <li class="nav-item mx-2">
                                <a class="btn btn-warning font-weight-bold px-3 py-2" href="{{ route('personal.main.index') }}"><i class="fas fa-user"></i></a>
                            </li>
                            @if (auth()->user()->role == 1)
                                <li class="nav-item">
                                    <a href="{{ route('admin.main.index') }}" class="btn btn-outline-warning font-weight-bold px-5 py-2">Адмін панель</a>
                                </li>
                            @endif
                        </ul>
                    @endauth
                    @guest
                        <ul class="navbar-nav mt-2 mt-lg-0">
                            <li class="nav-item mx-2">
                                <a class="btn btn-warning font-weight-bold px-3 py-2" href="{{ route('personal.main.index') }}">Увійти</a>
                            </li>
                            <li class="nav-item">
                                <a class="btn btn-warning font-weight-bold px-3 py-2" href="{{ route('personal.main.index') }}">Зареєструватись</a>
                            </li>
                        </ul>                        
                    @endguest
                </div>
